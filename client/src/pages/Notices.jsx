...
export default function Notices() {
    ...
    const [filter, setFilter] = useState("ALL"); // ALL, GOVERNMENT, AGRI_NEWS
    const [darkMode, setDarkMode] = useState(false); // Could context this, but keeping isolated

    useEffect(() => {
        // Check dark mode preference for initial render
        if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
            // logic to detect from parent or local storage would be better, 
            // but for now let's just default to light or use simple logic
        }
    }, []);
